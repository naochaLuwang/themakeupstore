...
// import { Minus, Plus, ShoppingBag, MapPin, Check, AlertCircle, Loader2, Truck, Sparkles, Search, Palette, ChevronRight, X } from "lucide-react"
...
//     // --- ORIGINAL LOGIC & STATE (UNTOUCHED) ---
...
//                                         <p className="text-[10px] font-bold text-slate-300 uppercase italic">Shade not found</p>
//                                     </div>
//                                 )}
//                             </AnimatePresence>
//                         </div>
...
//             {/* 3. PRIMARY ACTION AREA (OPTIMIZED SPACING) */}
//             <div className="space-y-4">
//                 <div className="flex items-center gap-4">
//                     {/* Ghost Style Quantity Selector */}
...
//                         {selectedVariant.stock <= 0 ? "Waitlist Only" : "Add to Bag"}
//                     </button>
//                 </div>
...
//             {/* 4. LOGISTICS SECTION (CLEANER SEPARATION) */}
//             <div className="pt-4">
...
//     // Determine if we should show the variant selector
...
//         hasVariants
//             ? (variants.find((v: any) => v.is_default) || variants[0])
//             : product // Fallback to base product for standard items
//     )
...
//     // Price Logic: Standard products use product.base_price, variants use selectedVariant.price
...
//     const handleAddToCart = () => {
//         const stockToCheck = hasVariants ? selectedVariant.stock : product.stock;
...
//             productId: product.id,
//             categoryId: product.category_id,
//             variantId: hasVariants ? selectedVariant.id : product.id,
//             name: product.name,
...
//             {/* 2. VARIANT PALETTE GRID - Conditional Rendering */}
//             {hasVariants && (
//                 <div className="bg-white rounded-[2.5rem] border border-pink-50 shadow-sm overflow-hidden">
//                     <div className="bg-slate-50/50 p-6 flex justify-between items-center border-b border-pink-50">
//                         <div className="space-y-1">
...
//                     <div className="flex items-center bg-slate-100/50 rounded-2xl p-1 h-14 border border-slate-100">
//                         <button
//                             disabled={quantity <= 1 || (hasVariants ? selectedVariant.stock : product.stock) <= 0}
//                             onClick={() => setQuantity(Math.max(1, quantity - 1))}
//                             className="w-10 h-10 flex items-center justify-center rounded-xl bg-white shadow-sm disabled:opacity-30 active:scale-95 transition-all"
//                         >
//                             <Minus className="w-4 h-4 text-slate-600" />
...
//                         <span className="w-10 text-center text-sm font-black tabular-nums">{quantity}</span>
//                         <button
//                             disabled={quantity >= (hasVariants ? selectedVariant.stock : product.stock) || (hasVariants ? selectedVariant.stock : product.stock) <= 0}
//                             onClick={() => setQuantity(Math.min((hasVariants ? selectedVariant.stock : product.stock), quantity + 1))}
//                             className="w-10 h-10 flex items-center justify-center rounded-xl bg-white shadow-sm disabled:opacity-30 active:scale-95 transition-all"
//                         >
//                             <Plus className="w-4 h-4 text-slate-600" />
//                         </button>
...
//                     <button
//                         disabled={(hasVariants ? selectedVariant.stock : product.stock) <= 0}
//                         onClick={handleAddToCart}
//                         className={`flex-1 h-14 rounded-2xl text-[11px] font-black uppercase tracking-[0.2em] flex items-center justify-center gap-3 transition-all shadow-xl
//                             ${(hasVariants ? selectedVariant.stock : product.stock) <= 0
//                                 ? "bg-slate-100 text-slate-400 cursor-not-allowed shadow-none"
//                                 : "bg-[#fc2779] text-white shadow-pink-200/50 hover:bg-pink-600 active:scale-[0.98]"}`}
//                     >
//                         <ShoppingBag className="w-4 h-4" />
//                         {(hasVariants ? selectedVariant.stock : product.stock) <= 0 ? "Waitlist Only" : "Add to Bag"}
//                     </button>
//                 </div>

//                 {(hasVariants ? selectedVariant.stock : product.stock) > 0 && (hasVariants ? selectedVariant.stock : product.stock) < 5 && (
//                     <p className="text-center text-[9px] font-black uppercase tracking-widest text-amber-600">
//                         Hurry! Only {(hasVariants ? selectedVariant.stock : product.stock)} items left in stock
//                     </p>
//                 )}
//             </div>
...
export default function VariantSelector({ product, variants = [], onVariantChange }: any) {
    ...
    // 1. Unified Stock Logic: Even standard products use the first variant's stock per your schema
    const hasVariants = product.has_variants && variants && variants.length > 0;
    ...
    const [selectedVariant, setSelectedVariant] = useState(() => {
        ...
        // Fallback for standard products: Use the first variant object to get stock/price
