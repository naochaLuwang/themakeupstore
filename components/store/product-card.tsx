...
import { Heart, ShoppingBag, Plus, X, Check, AlertCircle, Palette, Handbag } from "lucide-react"
                ...
                {showVariantSelector && (
                    <>
                        ...
                        <motion.div
                            ...
                            className="fixed bottom-0 left-0 right-0 bg-white rounded-t-[2.5rem] z-[601] p-6 pb-10 shadow-2xl max-w-lg mx-auto"
                        >
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={`w-full flex items-center justify-between p-4 rounded-2xl border transition-all group/item
                                                ${isVOut
                                                    ? 'opacity-40 border-slate-50 cursor-not-allowed bg-slate-50/50'
                                                    : 'border-slate-100 hover:border-slate-900 hover:bg-slate-50/50 hover:shadow-sm'}`}
                                        >
                                            <div className="flex items-center gap-5">
                                                {/* HEX COLOR SWATCH OVER IMAGE */}
                                                <div
                                                    className="h-8 w-8 rounded-lg border border-slate-100 flex items-center justify-center shadow-inner overflow-hidden"
                                                ...
                                                <div className="text-left space-y-0.5">
                                                    ...
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        {vSale < vBase && (
                                                            <span className="text-[10px] text-slate-400 line-through">₹{Math.round(vBase).toLocaleString()}</span>
                                                        )}
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            {isVOut ? (
                                                <span className="text-[9px] font-bold text-slate-300 uppercase">Archive</span>
                                            ) : (
                                                <div className="h-9 w-9 rounded-xl bg-slate-900 text-white flex items-center justify-center opacity-0 group-hover/item:opacity-100 transition-all scale-75 group-hover/item:scale-100">
