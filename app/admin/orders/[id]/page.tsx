...
//     // 1. Savings from individual product discounts (Difference between MRP and Unit Price)
//     const productSavings = order.order_items.reduce((acc: number, item: any) => {
//         const mrp = Number(item.mrp || item.unit_price);
//         const paid = Number(item.unit_price);
//         return acc + ((mrp - paid) * item.quantity);
//     }, 0);

//     // 2. Extra discount from the Promo Code (Order-level)
//     const promoDiscount = Number(order.promo_discount_amount) || 0;

//     // 3. Combined Total Savings (Products + Promo)
//     const totalSavings = productSavings + promoDiscount;

//     // 4. Subtotal (MRP) representing the "raw" value of items before any cuts
//     // Logic: (Final Total - Shipping) + All Savings
//     const subtotalMRP = (Number(order.total) - (Number(order.shipping_price) || 0)) + totalSavings;
...
//                     >
//                         <Ticket className="w-4 h-4 mr-2" /> {isThermal ? 'A4 Format' : 'POS Format'}
//                     </Button>
...
//                             <Truck className="w-2.5 h-2.5" /> {order.shipping_label || "Standard Delivery"}
//                         </p>
//                     </div>
//                 </div>
...
//                     {/* Subtotal representing original value */}
...
//                                 <Ticket className="w-2 h-2" /> {order.promo_code || 'PROMO'}
//                             </span>
//                             <span>-₹{promoDiscount.toLocaleString()}</span>
//                         </div>
//                     )}

//                     {/* Total Savings (Product + Promo) */}
//                     {totalSavings > 0 && (
...
//                     <QRCodeSVG value={`https://dacianastore.in/track/${id}`} size={isThermal ? 60 : 80} />
        ...
        <div className={`min-h-screen py-10 px-4 transition-colors duration-500 ${isThermal ? 'bg-zinc-200' : 'bg-white'}`}>
            ...
            >
                ...
                <div className={`pt-4 border-t-2 border-slate-900 border-dashed ${isThermal ? 'space-y-1' : 'flex flex-col items-end space-y-2'}`}>
                    ...
                    {/* 2. Item Savings (Only shows if products were discounted) */}
                    {productSavings > 0 && (
                    ...
                    {/* 3. CLARITY SUB-TOTAL (Items price after store discount) */}
