    ...
    const fetchProduct = async () => {
        try {
            ...
            addToRecentlyViewed({
                ...
                base_price: data.base_price, thumbnail_url: data.thumbnail_url,
                brand: data.brand, discount_type: data.discount_type,
                discount_value: data.discount_value, has_variants: data.has_variants,
                status: data.status, product_variants: data.product_variants || [],
            })

            let allImages: string[] = []
            const pi = data.product_images
            if (Array.isArray(pi)) {
                allImages = pi
                    .sort((a: any, b: any) => (a.position || 0) - (b.position || 0))
                    .map((img: any) => img.url || img.image_url || img.secure_url || img)
                    .filter(Boolean)
            }
            if (allImages.length === 0) {
                ...
                if (imagesData?.length) {
                    allImages = imagesData.map((img: any) => img.url).filter(Boolean)
                }
            }
            ...
            setProduct({ ...data, image_urls: allImages })

            if (data.product_reviews?.length > 0) {
                const total = data.product_reviews.length
                const avg = data.product_reviews.reduce((sum: number, r: any) => sum + r.rating, 0) / total
            ...
            if (data.category_id) similarCatIds.add(data.category_id)
    ...
    if (!product) {
        return (
            <div className="min-h-screen bg-white flex flex-col items-center justify-center gap-4 px-6">
                ...
                <button onClick={() => router.back()} className="text-[#fc2779] text-sm font-semibold">Go back</button>
            </div>
        )
    }
