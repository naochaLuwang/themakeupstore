...
export default async function CategoryPage({ params }: { params: Promise<{ slug: string }> }) {
    ...
    if (productIds.length > 0) {
        const { data: products } = await supabase
            .from("products")
            .select("*, product_variants(id, price, stock, hex_code, discount_type, discount_value, title, image_url), product_concerns(concern_id), product_categories(category_id)")
            .eq("status", "active")
            .in("id", productIds)
